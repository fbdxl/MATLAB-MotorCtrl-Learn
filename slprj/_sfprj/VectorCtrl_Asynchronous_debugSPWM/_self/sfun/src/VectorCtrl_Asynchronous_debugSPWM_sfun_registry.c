...
{
  extern unsigned int
    sf_VectorCtrl_Asynchronous_debugSPWM_process_check_sum_call( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_VectorCtrl_Asynchronous_debugSPWM_process_check_sum_call(nlhs,plhs,nrhs,
       prhs))
    return 1;
  return 0;
}
...
{
  extern unsigned int sf_VectorCtrl_Asynchronous_debugSPWM_autoinheritance_info(
  ...
  if (strcmp(machineName, "VectorCtrl_Asynchronous_debugSPWM") == 0) {
    const mxArray *newRhs[3] = { NULL, NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    newRhs[2] = prhs[3];
    return sf_VectorCtrl_Asynchronous_debugSPWM_autoinheritance_info(nlhs,plhs,3,
      newRhs);
  }
...
{
  extern unsigned int
    sf_VectorCtrl_Asynchronous_debugSPWM_get_eml_resolved_functions_info( int
  ...
  if (strcmp(machineName, "VectorCtrl_Asynchronous_debugSPWM") == 0) {
    ...
    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_VectorCtrl_Asynchronous_debugSPWM_get_eml_resolved_functions_info
      (nlhs,plhs,2,newRhs);
  }
...
extern unsigned int sf_VectorCtrl_Asynchronous_debugSPWM_method_dispatcher
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"VectorCtrl_Asynchronous_debugSPWM")) {
    return(sf_VectorCtrl_Asynchronous_debugSPWM_method_dispatcher(simstructPtr,
...
extern void VectorCtrl_Asynchronous_debugSPWM_terminator(void);
void sf_machine_global_terminator(void)
{
  if (sfMachineGlobalTerminatorCallable) {
    ...
    VectorCtrl_Asynchronous_debugSPWM_terminator();
    sf_debug_terminate();
  }
...
extern void VectorCtrl_Asynchronous_debugSPWM_initializer(void);
extern void VectorCtrl_Asynchronous_debugSPWM_register_exported_symbols
  (SimStruct* S);
extern void VectorCtrl_Asynchronous_debugSPWM_debug_initialize(void);
void sf_register_machine_exported_symbols(SimStruct* S)
{
  VectorCtrl_Asynchronous_debugSPWM_register_exported_symbols(S);
}
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      VectorCtrl_Asynchronous_debugSPWM_debug_initialize();
    }

    VectorCtrl_Asynchronous_debugSPWM_initializer();
  }
