...
TaoiStep=0.0002;
str=[];
fpiBox=zeros(1,TaoiEnd/TaoiStep+1);
fcutBox=zeros(1,TaoiEnd/TaoiStep+1);
PMBox=zeros(1,TaoiEnd/TaoiStep+1);
n=1;
for Taoi=TaoiStart:TaoiStep:TaoiEnd
    ...
    hline.DisplayName = strcat('{\tau}_i=',num2str(Taoi));
    %axis([-inf,inf,-inf,35]);
...
%     
%     %yrange=get(hgca_Phase,'Ylim');
%     semilogx(hgca_Phase,[fcut,fcut],[-180,GcPhase(indexTmp)],'b');%标注PM
   
end
    %legend(hgca_Mag);
    
    yrange=get(hgca_Mag,'Ylim');
    for n=1:TaoiEnd/TaoiStep+1
        %if ~isempty(fpiBox(n))
        if fcutBox(n) ~= 0
    ...
    xlimPhase=get(hgca_Phase,'Xlim');
    semilogx(hgca_Phase,[xlimPhase(1),xlimPhase(2)],[-180,-180],'r--');%标注-180°线
    semilogx(hgca_Phase,[xlimPhase(1),xlimPhase(2)],[-90,-90],'r--');%标注-90°线
    for n=1:TaoiEnd/TaoiStep+1
        if PMBox(n)~=0
