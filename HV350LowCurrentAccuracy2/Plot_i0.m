...
OSCData = readmatrix('dq_abc_current_harmonic_3sample_50Hz.csv', 'Range', [100,2,100+FFTLength-1,6]);
OSCData(:,6) = (OSCData(:,1)+OSCData(:,2)+OSCData(:,3))/3;
...
figure('Position',[500 400 400 150 ]);
subplot(1,1,1);
plot(f(2:end), P1(2:end,6),'LineWidth',1); grid on; hold on;  
% title('Isc AD原始值频谱 2kHz Fsw 50Hz发波');
% xlabel('f (Hz)');
% ylabel('幅值');
% set(get(gca,'XLabel'),'FontSize',FontSizeMy,'FontName',FontNameMy);
